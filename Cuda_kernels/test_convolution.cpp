...
TEST_CASE("Check cudnn full convolution") {
  ...
  Tensor<float> cpu_input(dN, dC, dH, dW);
  Tensor<float> cpu_output(cpu_input);
  Tensor<float> cpu_kernel = Tensor<float>(dC, dF, dKH, dKW);

  random_fill(len_input, cpu_input.data);
  random_fill(len_output, cpu_output.data);
  random_fill(len_kernel, cpu_kernel.data);

  Tensor<float, CudaAllocator> gpu_input(cpu_input);
  Tensor<float, CudaAllocator> gpu_output(cpu_output);
  Tensor<float, CudaAllocator> gpu_kernel = cpu_kernel;
