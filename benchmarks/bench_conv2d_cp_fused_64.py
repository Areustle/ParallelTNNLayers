...
def create_op_pairs():
    ...
    #     print(i, factor.shape)

    K0 = factors[0]
    # K1 = np.reshape(factors[1], (shape[0], shape[1], rank, 1))
    K1 = np.reshape(factors[1], (shape[0], shape[1], rank))
    K2 = np.transpose(factors[2])
    # K2 = np.reshape(np.transpose(factors[2]), (1, 1, rank, shape[3]))
    K2 = np.reshape(K2, (6, 16))
    U = np.random.normal(0., 1., [64,16,32,32]).astype(np.float32)
...
if __name__ == "__main__":
    ...
    cp_op_module = tf.load_op_library('../Kernels/cp_fused_nchw_64.so')

    # print(U.shape, K0.shape, K1.shape, K2.shape)
    ...
    with tf.Session() as sess:
        with tf.device('/device:GPU:0'):
            ...
            CPbench.run_op_benchmark(sess, V_custom, name='custom_op', min_iters=40)
            CPbench.run_op_benchmark(sess, V_orig, name='TF_op', min_iters=40)
