...
/*         C_local[(((x_c_init * 8) + (y_c_init * 2)) + r_c_init)] = 0.000000e+00f; */
/*       } */
/*     } */
/*   } */
/*   for (int sdim_outer = 0; sdim_outer < 32; ++sdim_outer) { */
/*     __syncthreads(); */
/*     U_shared[((((int)threadIdx.z) * 2) + ((int)threadIdx.y))] = U[((((((((int)blockIdx.x) * 16384) + ((((int)threadIdx.z) / 8) * 4096)) + (((int)blockIdx.y) * 512)) + ((((int)threadIdx.z) % 8) * 64)) + (sdim_outer * 2)) + ((int)threadIdx.y))]; */
